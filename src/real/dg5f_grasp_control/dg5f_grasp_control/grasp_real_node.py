    ...
    def _print_status(self, output, gravity, friction, effort, qdot):
        if output.state == "GROPED_GRASP":
            ...
            print(
                ...
                f"position_tau_max="
                f"{np.max(np.abs(output.relative_translation_position_torques)):.4f} | "
                f"null_grasp_tau_max="
                f"{np.max(np.abs(output.relative_translation_nullspace_grasp_torques)):.4f} | "
                f"dls_sigma_min="
                f"{output.relative_translation_dls_sigma_min:.5f} | "
                f"dls_condition="
                f"{output.relative_translation_dls_condition:.2f} | "
