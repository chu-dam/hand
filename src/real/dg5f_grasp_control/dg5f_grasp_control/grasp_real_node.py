...
class GraspRealRunner:
    def __init__(self, node, cfg, model_xml_path):
        ...
        initial_count = cfg.use_finger_count if cfg.use_finger_count > 0 else 1
    ...
    def command_cb(self, msg):
        ...
        if count < 0 or count > 5:
            self.node.get_logger().warn(
                f"Ignore invalid finger_count command: {count}. Use 0, 1, 2, 3, 4, or 5."
            )
            return
            ...
            while rclpy.ok():
                ...
                if self.pending_finger_count is not None:
                    ...
                    if requested_count == 0:
                        ...
                        prev_fingers = set(self.use_fingers) if self.active_finger_count > 0 else set()
